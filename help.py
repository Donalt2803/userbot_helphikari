...
# <3 desc: Cute help with Japan and foxes
...
class HelpMod(loader.Module):
    ...
    strings = {
        ...
        "single_cmd": "\n🧊 <code>{}{}</code> 👉🏻 ",
        "undoc_cmd": "❌ Нет документации",
        "all_header": '🔥 <b>{} модулей установленно:</b>',
        "mod_tmpl": '\n✨ <code>{}</code>',
        "first_cmd_tmpl": ": ( {}",
        "cmd_tmpl": " | {}",
        "args": "❌ <b>Неправельный аргумент</b>",
        "set_cat": "🐱 <b>{} -> {}</b>"
    }
            ...
            for mod in self.allmodules.modules:
                if len(mod.commands) != 0:
                    ...
                    if commands:
                        ...
                        reply = '<i>Доступные вам модули:</i>\n' + reply
                        shown_warn = True
            if category is None:
